...
<div class="product-container category-side-bar-container">
  ...
  const isLoggedIn = <?php echo isset($_SESSION['customer_email']) ? 'true' : 'false'; ?>;

  // Handle redirection if user is not logged in
  function handleButtonClick(event, redirectUrl) {
    if (!isLoggedIn) {
      // Prevent the default action (form submission) and redirect to the login page
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addToCartButton) {
      addToCartButton.addEventListener('click', function(event) {
        handleButtonClick(event, 'login.php'); // Redirect to login page
      });
    }
...
<button type="submit" id="addToCartButton" name="add_to_cart" class="btn_product_cart">
  Add to Cart
</button>

<button type="button" id="buyNowButton" class="btn_but_product">
  <a href="checkout.php" style="text-decoration:none; color:#FFFFFF">Buy Now</a>
</button>
