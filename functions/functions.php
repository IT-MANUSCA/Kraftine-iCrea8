<?php
    ...
    function get_category_bar_products(){
        global $conn;
        $query = "SELECT * FROM category_bar";
    ...
    function get_anniversary_category(){
        global $conn;
        $query = "SELECT * FROM anniversary WHERE anniversary.anniversary_category_status = 1";

        return $result = mysqli_query($conn, $query);
    }
    // get footwear category
    function get_wedding_category(){
        global $conn;
        $query = "SELECT * FROM wedding WHERE wedding.wedding_category_status = 1";

        return $result = mysqli_query($conn, $query);
    }
    // get jewelry category
    function get_sympathy_category(){
        global $conn;
        $query = "SELECT * FROM sympathy WHERE sympathy.sympathy_category_status = 1";

        return $result = mysqli_query($conn, $query);
    }
    // get perfume category
    function get_mothers_day_category(){
        global $conn;
        $query = "SELECT * FROM mothers_day WHERE mothers_day.mothers_day_category_status = 1";

        return $result = mysqli_query($conn, $query);
    }
    // get cosmetics category
    function get_fathers_day_category(){
        global $conn;
        $query = "SELECT * FROM fathers_day WHERE fathers_day.fathers_day_category_status = 1";

        return $result = mysqli_query($conn, $query);
    }
    // get glasses category
    function get_valentines_day_category(){
        global $conn;
        $query = "SELECT * FROM valentines_day WHERE valentines_day.valentines_day_category_status = 1";

        return $result = mysqli_query($conn, $query);
    }
    // get bags category
    function get_birthday_category(){
        global $conn;
        $query = "SELECT * FROM birthday WHERE birthday.birthday_category_status = 1";
...
// For example, if you want to select rows 3 to 7 from a table, you would replace (n - 1) with (3 - 1) and (m - n + 1) with (7 - 3 + 1). This would result in OFFSET 2 LIMIT 5. This query will retrieve the rows within the specified range from the table.
    ...
    function get_deal_of_day(){
        global $conn;
        $query = "SELECT * FROM deal_of_the_day";
    ...
    function display_memorial_day(){
        global $connect;
        $query = "SELECT * FROM category__memorial_day WHERE category__memorial_day.status = 1";
    ...
    function get_items_by_category_items($category){
        global $conn;
        $query = "SELECT * FROM products WHERE products.product_catag = '$category'";
